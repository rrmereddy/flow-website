...
export function AuthProvider({ children }: { children: React.ReactNode }) {
      ...
      if (firebaseUser) {
        try {
          ...
          if (userDoc.exists()) {
            ...
            if (
              userRole === 'user' &&
              userData.accountStatus === 'awaiting verification' &&
            ...
            if (
              ...
              await batch.commit()
              logger.log(
                'Driver email verified — updated user/drivers status and checklist'
              )
            }
      ...
      if (userDoc.exists()) {
        ...
        if (userRole === 'admin') {
          ...
          if (driverDoc.exists()) {
            ...
            if (
              driverData.checklist.accountInfo &&
              driverData.checklist.emailVerified &&
              driverData.checklist.driverDocs &&
              driverData.checklist.vehicleDocs &&
              driverData.checklist.paymentInfo
            ) {
              ...
              toast.info(
                'Please complete the driver registration process to continue'
              )
            }
          } else {
  ...
  return (
    <AuthContext.Provider
      value={{ user, loading, signup, login, logout, forgotPassword }}
    >
      {children}
    </AuthContext.Provider>
  )
...
async function validateReferralCode(referralCode: string) {
  ...
  const referralQuery = query(
    collection(db, 'referal_codes'),
    where('name', '==', referralCode.toLowerCase())
  )
