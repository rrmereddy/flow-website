...
export default function DriverRegistrationPage() {
  ...
  const handleVehicleInfoChange = (
    field: keyof typeof vehicleInfo,
    value: string
  ) => {
  ...
  const saveDocumentsToDB = async () => {
    try {
      ...
      const profilePictureURL = await uploadImageAsync(
        documents.profilePicture,
        `users/${user.uid}`,
        'profilePicture'
      )
      const licenseImageURL = await uploadImageAsync(
        documents.license,
        `drivers/${user.uid}/documents`
      )
      const vehicleRegistrationImageURL = await uploadImageAsync(
        documents.vehicleRegistration,
        `drivers/${user.uid}/documents`
      )
  ...
  const setupStripe = async () => {
    ...
    try {
      ...
      const errorMessage =
        (error as Error)?.message || 'Could not connect to Stripe.'
  ...
  const renderDocumentsSection = () => (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Upload your profile picture, driver&apos;s license, and vehicle
          registration
        </CardDescription>
      </CardHeader>
      <CardContent>
        <FieldGroup>
          <Field>
            ...
            <Input
              ...
              onChange={(e) =>
                e.target.files?.[0] &&
                handleDocumentUpload('profilePicture', e.target.files[0])
              }
            />
            <FieldDescription>
          ...
          <Field>
            ...
            <Input
              ...
              onChange={(e) =>
                e.target.files?.[0] &&
                handleDocumentUpload('vehicleRegistration', e.target.files[0])
              }
            />
            <FieldDescription>
  ...
  const renderVehicleInfoSection = () => (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Provide details about your vehicle</CardDescription>
      </CardHeader>
      <CardContent>
        <FieldGroup>
      ...
      <CardContent>
        <FieldGroup>
          <Field>
            ...
            <FieldDescription>
              We&apos;ll help you set up a Stripe Connect account to receive
              payments from riders. This process includes identity verification
          ...
          {stripeSetup.onboardingComplete && (
            <div className="mb-4 rounded-lg bg-green-50 p-4">
              ...
              <p className="text-sm text-green-800">
                Please complete the Stripe setup process in the opened window.
                Once finished, you&apos;ll be redirected back here.
              </p>
            </div>
          )}
  ...
  const renderCompletionSection = () => (
    <Card>
      <CardContent className="py-12 text-center">
        ...
        <h3 className="mb-2 text-xl font-semibold">Registration Submitted!</h3>
        <p className="mx-auto mb-6 max-w-md text-gray-600">
          Your account is under review. Download the Flow app to track your
          application status.
        </p>
        <div className="mx-auto flex h-32 w-32 items-center justify-center rounded-lg bg-gray-200">
          <Image
            src="/Flow_QR.png"
            alt="Flow App QR Code"
            width={128}
            height={128}
          />
        </div>
