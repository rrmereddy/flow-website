...
export function PrototypeCarousel() {
  ...
  React.useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      const newImageIndex = Math.floor(
        (elapsedTime % carouselDuration) / singleImageDuration
      )
  ...
  return (
    <div className="flex w-full max-w-xs flex-col items-center overflow-hidden">
      <div className="relative w-full overflow-hidden">
        <motion.div className="flex" initial={{ x: 0 }} animate={controls}>
          {[...images, ...images].map((name, index) => (
            <div
              key={index}
              className="flex w-full flex-shrink-0 justify-center p-2"
            >
              <Iphone15Pro className="h-auto w-full" src={name} />
            </div>
          ))}
        </motion.div>
      </div>
